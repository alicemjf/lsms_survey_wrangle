...
# this is for 5 countries, for a single year (TZA LSMS 2008, MWI LSMS 2010, ETH LSMS, NGA LSMS, NER LSMS, UGA LSMS)
# this builds on the script 3.e2eWIP which works for one country, one year
...
col_lookup <- read.csv('./lookup/col_lookup0810.csv')
...
# country code is taken from the first 3 letters of the object name
myfiles <- mapply(cbind, myfiles, "country"=substr(names(myfiles),0,3), SIMPLIFY=F)

# add a column to each df in the list contains the survey year
# year is taken from the last 4 letters of the object name 
# called af_year because some files already have "year" columns in them
myfiles <- mapply(cbind, myfiles, "af_year"=str_sub(names(myfiles),-4,-1), SIMPLIFY=F)
...
myfiles <- lapply(myfiles, transform, hhid2=ifelse(country == "NER", paste0(GRAPPE, MENAGE, EXTENSION), hhid))



# add a column that is country_hhid to create a new hhid 
myfiles <- lapply(myfiles, transform, af_hhid = paste0(country, "_", hhid2))

# convert all vars in all dfs to as.character 
...
by_indiv_join <- by_indiv_join %>%
  mutate(tmp = case_when(relation_head %in% labels$label ~ labels$new_label[match(by_indiv_join$relation_head, labels$label)],
                          TRUE ~ relation_head)) # create a temp column, if the label is in the lookup table, return the new lable, otherwise leave as is
by_indiv_join <- by_indiv_join %>%
...
mydf <- left_join(by_hhid_join, summary, by = "af_hhid")
...
# apply look up to cooking fuel labels
mydf <- mydf %>%
