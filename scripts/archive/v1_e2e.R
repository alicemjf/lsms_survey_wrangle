# OVERVIEW #
# this script is the first working version for the end-to-end (e2e) automated wrangling & cleaning for household survey data.
# This script does the following steps (MULTIPLE countries, for MULTIPLE years)
  #> Reads in .dta files based on those within a given lookup table ('col_lookup_panel.csv')
  #> Renames, en masse, the column names for the variables of interest (as listed in 'vars.csv', renaming is given in 'col_lookup_panel.csv')
  #> Create a final df, which contains all vars of interest, including vars calculated from indiv to hh level (dependency ratio, hh size, sex ratio ect..)

# this builds on the script 5.e2eWIP (saved in archive) which works for multiple country, multiple years
# this script does exactly the same as 5e2e but is a WIP for turning some of the code into functions.
...
# first add a column to each df in the list contains the survey year
myfiles <- mapply(cbind, myfiles, "yr"=str_sub(names(myfiles),-4,-1), SIMPLIFY=F) 

# then need to prefix each column name with year so that column name look ups are year specific
# in lookup 
col_lookup <- col_lookup %>%
  mutate(orig_col_id = paste0(year, "_", orig_col))

# in list of survey dataframes
...
myfiles2 <- unlist(unname(myfiles2), recursive = FALSE) # unname prevents nested list name being appended to the unlisted lists


###################################################. 5. ADD META DATA TO EACH FILE ##########################################################

# add a column to each df in the list contains the country code
myfiles2 <- mapply(cbind, myfiles2, "country"=substr(names(myfiles2),0,3), SIMPLIFY=F) # country code is taken from the first 3 letters of the object name

# add a column to each df in the list contains the survey year
myfiles2 <- mapply(cbind, myfiles2, "af_year"=str_sub(names(myfiles2),-4,-1), SIMPLIFY=F) # year is taken from the last 4 letters of the object name - called af_year because some files already have "year" columns in them
...
myfiles2 <- lapply(myfiles2, transform, hhid2=ifelse(country == "NER" & af_year == "2014", paste0(`2014_GRAPPE`, `2014_MENAGE`, `2014_EXTENSION`), hhid))

# add a column that is country_hhid to create a new hh survey id - unique to each hh in a survey year
myfiles2 <- lapply(myfiles2, transform, af_hhid = paste0(country, "_", hhid2, "_", af_year)) 
...
by_hhid_join <- by_hhid_join %>%
  group_by(af_hhid) %>% # NEED TO GROUP BY YEAR TO AVOID GROUPING HH'S ACROSS YEARS
...
by_indiv_join$sex <- toupper(by_indiv_join$sex)
by_indiv_join$age <- as.numeric(by_indiv_join$age)
...
mydf <- left_join(by_hhid_join, hh_summary, by = "af_hhid")

# re-order columns
mydf <- dplyr::select(mydf, af_hhid, country, af_year, urb_rur, hh_head_sex, hh_size, depend_ratio, dist_road, cooking_fuel_prm) 
