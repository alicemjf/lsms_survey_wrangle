...
ner18_housing_t <- ner18_housing %>% 
  ...
  mutate(collected_wood = s11q53__1,  # rename cols
         ...
         other = s11q53__8) %>%
  dplyr::select(- c(s11q53__1, s11q53__2, s11q53__3, s11q53__4, s11q53__5, s11q53__6, s11q53__7, s11q53__8))

ner18_housing_l <- ner18_housing_t %>% 
  pivot_longer(                            # pivot rows to columns
    !c(grappe, menage)                          ## use all columns except hhid
  ) %>%
...
########################
 # to look at secondar cooking fuel

ner18_housing_l2 <- ner18_housing_t %>% 
  pivot_longer(                            # pivot rows to columns
    !c(grappe, menage)                          ## use all columns except hhid
  ) %>%
  filter(value == 2) %>% # filter for 1 - this means it is the primary fuel
  mutate(prm_cooking_fuel = name) %>%
  dplyr::select(-c(name, value))
